...
from src.datasets.medical_data import Json
from src.datasets.tokeniser import Tokeniser
...
class JSONKeyValueResnet_EmbeddingEncoder(nn.Module):
    """
    For explanation of EmbeddingEncoder see JSONKeyValueModel_EmbeddingEncoder

    This model implements a 1D (sequence) ResNet and defaults to a ResNet50
    ...
    def preprocessing(self, json_input: Json) -> torch.LongTensor:
        """
        1. flatten design summary into a list of key and values
        ...
        flattened_json_input: List[Tuple[str, str]] = list(
            JSONKeyValueResnet_EmbeddingEncoder.parse_json_into_only_fields(json_input)
        )
        if not flattened_json_input:
            logger.warning(f"No json extracted from {repr(json_input)}")
            flattened_json_input = [("empty", "empty")]
        # embed the whole key "a.b.c" -> [10]th index for example
        # if "a.b.c" maps to 10th index in the vocabulary
        keys = (
            torch.LongTensor([self.tokeniser.convert_token_to_id(key)])
            for key, _ in flattened_json_input
        )

        # logger.info(f"length of input = {len(flattened_json_input)}")
        # but embed values as a list of runes "100.0" -> [11, 12, 12, 13, 12] for example, if
        # 11 -> '1'
        # 12 -> '0'
        # 13 -> '.'
        values = (
        ...
        # if len(output) > 16384:
        #     logger.warning(f'{len(output)} >= 16384')
        return output
